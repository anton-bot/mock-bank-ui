...
import { generateRandomAccountNumber, getAccounts, getStorage, getTransactionDescription, initializeNewUser, initializeStorage, transfer } from "./fakebank";
...
describe('Mock banking backend based on localStorage', () => {
    ...
    it('must transfer money from one account to another', () => {
        ...
        const secondAccountId = accounts.find(a => a.currency === currency && a.accountId !== firstAccountId)?.accountId;
        if (!firstAccountId || !secondAccountId) {
            throw new Error(`Can't find an account for ${currency}: ${firstAccountId} / ${secondAccountId}`);
        }
